...
class RBDLModel:
    ...
    def calc_body_to_base_coordinates(
        self, q, body, coordinates_in_body_frame=np.zeros(3)
    ):
        r"""
        ...
        """
        p = rbdl.CalcBodyToBaseCoordinates(
            self.model,
            q,
            body.value,
            coordinates_in_body_frame,
            update_kinematics=self.uk,
        )
        return p

    def calc_base_to_body_coordinates(
        self, q, body, coordinates_in_base_frame=np.zeros(3)
    ):
        r"""
        ...
        """
        p = rbdl.CalcBaseToBodyCoordinates(
            self.model,
            q,
            body.value,
            coordinates_in_base_frame,
            update_kinematics=self.uk,
        )
        return p
    ...
    def calc_body_orientation(self, q, body):
        ...
        """
        return rbdl.CalcBodyWorldOrientation(
            self.model, q, body.value, update_kinematics=self.uk
        ).T
    ...
    def calc_body_Jacobian(self, q, body):
        ...
        rbdl.CalcBodySpatialJacobian(
            self.model, q, body.value, np.zeros(3), J, update_kinematics=self.uk
        )
        return J
    ...
    def calc_point_acceleration(
        self, q, qdot, qddot, body, coordinates_in_body_frame=np.zeros(3)
    ):
        r"""
        ...
        """
        acc = rbdl.CalcPointAcceleration(
            self.model,
            q,
            qdot,
            qddot,
    ...
    def calc_center_of_mass_position(self, q, qdot):
        ...
        mass = rbdl.CalcCenterOfMass(
            self.model, q, qdot, None, com, update_kinematics=self.uk
        )
        return mass, com
    ...
    def calc_zero_moment_point(
        self, q, qdot, qddot, plane_normal=np.array([0, 1.0, 0])
    ):
        r"""
