...
class RNN(torch.nn.Module):
    ...
    def __init__(
        ...
        """
        super().__init__()
        self.rnn = getattr(torch.nn, rnn_type.upper())(
            hidden_size,
            hidden_size,
            num_rnn_layer,
...
class RNNWithInit(RNN):
    ...
    def __init__(
        ...
        self.init_net = torch.nn.Sequential(
            ...
            torch.nn.ReLU(),
            torch.nn.Linear(
                hidden_size * num_rnn_layer,
                2 * (2 if bidirectional else 1) * num_rnn_layer * hidden_size,
            ),
        )
    ...
    def forward(self, x, _=None):
        ...
        h, c = (
            self.init_net(torch.stack(x_init)).view(-1, 2, nd, nh).permute(1, 2, 0, 3)
        )
