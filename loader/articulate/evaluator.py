...
class FullMotionEvaluator(BasePoseEvaluator):
    ...
    def __call__(
        ...
        lae = radian_to_degree(
            angle_between(pose_local_p, pose_local_t).view(pose_p.shape[0], -1)
        )  # N, J
        gae = radian_to_degree(
            angle_between(pose_global_p, pose_global_t).view(pose_p.shape[0], -1)
        )  # N, J
        jkp = (
...
class IntegratedEvaluator(BasePoseEvaluator):
    def __init__(
        ...
        :param device: torch.device, cpu or cuda.
        """
        super(IntegratedEvaluator, self).__init__(
            official_model_file, rep, use_pose_blendshape, device=device
        )
        self.align_joint = 0 if align_joint is None else align_joint.value
        self.fps = fps
    ...
    def __call__(
        ...
        gae = radian_to_degree(
            angle_between(
                pose_global_p.view(pose_p.shape[0], -1),
                pose_global_t.view(pose_p.shape[0], -1),
            ).view(pose_p.shape[0], -1)
        )  # N, J
        jkp = (
            (joint_p[3:] - 3 * joint_p[2:-1] + 3 * joint_p[1:-2] - joint_p[:-3])
            * (f**3)
